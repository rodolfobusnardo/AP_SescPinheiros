...
// require_once 'utils.php'; // If any utilities are needed
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        ...
        $sql_update_item = "UPDATE items SET status = 'Devolvido' WHERE id = ? AND status = 'Pendente'";
        ...
        // Note: This creates one document per item. If one document per transaction is desired, schema would differ.
        ...
        // If signature was saved but DB failed, consider deleting the orphaned signature file (or handle via cron job)
    ...
    if ($all_successful) {
        $_SESSION['home_page_message'] = count($processed_item_ids) . ' item(s) devolvido(s) com sucesso!';
        // Redirect to a success page or home page. Later, perhaps to manage_devolutions.php
        header('Location: home.php?success=devolution_complete&count=' . count($processed_item_ids));
        exit();
    }
...
// $conn->close(); // Usually closed automatically at end of script
?>
