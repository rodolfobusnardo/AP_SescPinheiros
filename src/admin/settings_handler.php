...
if ($check_stmt) {
    ...
    if ($check_result->num_rows === 0) {
        ...
        // Note: The VALUES part should have 9 placeholders if config_id is also a param, but it's fixed in this table structure.
        // So, 8 placeholders for the 8 data fields.
        $insert_stmt = $conn->prepare("INSERT INTO settings (config_id, unidade_nome, cnpj, endereco_rua, endereco_numero, endereco_bairro, endereco_cidade, endereco_estado, endereco_cep) VALUES (1, ?, ?, ?, ?, ?, ?, ?, ?)");
        if ($insert_stmt) {
            $insert_stmt->bind_param( // 8 string parameters for the 8 fields
                "ssssssss",
...
// Bind parameters (8 parameters: ssssssss)
$stmt->bind_param(
    "ssssssss",
    $unidade_nome,
    $cleaned_cnpj,
