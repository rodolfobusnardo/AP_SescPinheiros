...
try {
    ...
    if (empty($institution_cnpj)) $errors[] = "CNPJ da instituição é obrigatório."; // Added validation for CNPJ
    ...
    // Define o diretório de upload físico relativo à raiz do documento.
    $document_root = $_SERVER['DOCUMENT_ROOT']; // Geralmente /var/www/html no contêiner app
    $physical_upload_dir = $document_root . '/uploads/signatures/';

    // Garante que o diretório de upload físico exista
    if (!is_dir($physical_upload_dir)) {
        if (!mkdir($physical_upload_dir, 0775, true)) { // Tenta criar recursivamente
             error_log("Failed to create signature upload directory: " . $physical_upload_dir);
    ...
    $signature_file_path_full = $physical_upload_dir . $signature_filename; // Caminho completo no sistema de arquivos

    // Caminho a ser salvo no banco de dados (relativo à raiz do site para URLs)
    $signature_db_path = 'uploads/signatures/' . $signature_filename;

    if (!file_put_contents($signature_file_path_full, $base64_data)) {
        error_log("Failed to save signature image to: " . $signature_file_path_full . " (check permissions and path validity)");
    ...
    // B. Insert into donation_terms (simplificado)
    $sql_insert_term = "INSERT INTO donation_terms (
        user_id, responsible_donation, donation_date, donation_time,
        institution_name, institution_responsible_name, signature_image_path, status
    ) VALUES (?, ?, ?, ?, ?, ?, ?, 'Aguardando Aprovação')";
    $stmt_insert_term = $conn->prepare($sql_insert_term);
    if (!$stmt_insert_term) throw new Exception("Erro ao preparar inserção do termo simplificado: " . $conn->error);

    $stmt_insert_term->bind_param("issssss",
